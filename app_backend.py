...
class Model_Generation:
    ...
    def naive_NN_classifier(self, step):
        ...
        output_hid_2 = Dense(700, kernel_initializer='random_normal',
                             activation='relu')(output_hid_1)
        predictions = Dense(num_classes, kernel_initializer='random_normal',
                            activation='softmax')(output_hid_2)
    ...
    def find_input_output_data_by_date(self, model_abbr, date, step, scaler=MinMaxScaler()):
        ...
        if model_abbr == "LSTM":
            X_in = feature_matrix_rescaled.reshape(feature_matrix_rescaled.shape[0], step, int(
                feature_matrix_rescaled.shape[1] / step))[index]
